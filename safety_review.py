...
_WORK_TYPE_KEYWORDS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("受限空间", ("受限空间", "有限空间", "罐内", "池内", "井下", "进入罐体", "进入储罐", "储罐内部", "罐体内部", "进入设备内部")),
    ("动火作业", ("动火", "焊接", "切割", "打磨", "明火")),
    ("高处作业", ("高处", "登高", "脚手架", "屋顶", "高空")),
    ("吊装作业", ("吊装", "起重", "吊车", "行车")),
    ("临时用电", ("临时用电", "临电", "配电箱", "带电")),
    ("动土/开挖", ("动土", "开挖", "挖掘", "破土")),
    ("检维修与能量隔离", ("检修", "维修", "检维修", "能量隔离", "能源隔离", "上锁挂牌", "loto", "断电", "停电", "切断电源", "隔离电源", "拆卸管线", "打开管线", "管线打开", "法兰拆卸", "拆线", "拆卸线路")),
    ("危化品作业", ("危化品", "化学品", "酸洗", "hf", "氢氟酸", "盐酸", "硫酸", "甲醇", "methanol", "溶剂", "储罐")),
)

_RISK_KEYWORDS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("化学品暴露", ("化学品", "危化品", "hf", "氢氟酸", "盐酸", "硫酸", "酸洗", "溶剂", "甲醇", "methanol", "有机溶剂", "残液")),
    ("腐蚀/灼伤", ("酸", "碱", "腐蚀", "灼伤", "飞溅")),
    ("火灾爆炸", ("动火", "焊", "切割", "易燃", "爆炸", "可燃", "易燃蒸气", "防静电", "点火源")),
    ("中毒窒息", ("有毒", "窒息", "缺氧", "受限空间", "有限空间", "罐内", "通风", "气体检测", "置换", "有毒气体")),
    ("高处坠落", ("高处", "登高", "脚手架", "高空", "屋顶")),
    ("触电", ("用电", "带电", "配电", "电气", "电缆", "断电", "停电", "切断电源", "隔离电源", "拆线", "拆卸线路", "电气检修")),
    ("机械伤害", ("机械", "转动", "夹伤", "切割机", "设备检修", "检维修", "更换阀门", "拆卸")),
    ("压力/能量释放", ("压力", "蒸汽", "能量隔离", "能源隔离", "上锁挂牌", "loto", "泄压", "残余压力", "管线打开", "拆卸管线", "打开管线", "管线开口", "法兰拆卸", "置换")),
    ("起重与物体打击", ("吊装", "起重", "吊车", "物体打击")),
    ("受限空间", ("受限空间", "有限空间", "罐内", "池内", "井下", "进入罐体", "进入储罐", "储罐内部", "罐体内部", "进入设备内部")),
    ("环境泄漏", ("泄漏", "溢出", "排放", "环境", "残液", "管线打开", "打开管线", "法兰拆卸")),
    ("交叉作业", ("交叉作业", "同时作业", "多工种")),
    ("承包商作业", ("承包商", "外包", "施工方")),
)
...
def pack_blockers(draft: Mapping[str, Any], pack: Mapping[str, Any]) -> list[str]:
    ...
    for conflict in pack.get("conflicts") or ():
        ...
        if not conflict.get("resolved"):
            blockers.append(
                _text(conflict.get("text"))
                or _text(conflict.get("description"))
                or "存在未处理的资料冲突"
            )
